...
class MainCoordinator: NSObject, UINavigationControllerDelegate, Coordinator, NetworkCheckObserver, LocationDelegate {
    ...
    func locationReadyGetData(location: State) {
        ...
        CloudKitDataHandler.instance.getData(locationFound: location, dataFetched: { produce in
            self.statusViewModel?.status.location = location
            DispatchQueue.main.async {
                // send data to coordinator
                self.viewModel = ProduceCellViewModel.init(produceData: produce)
                print(self.firstRun)
                if self.firstRun == false {
                    self.presentMonthVC()
                } else {
                    if self.welcomeVC != nil {
                        self.welcomeVC?.startStopAnimations(start: false)
                        self.welcomeVC?.dismissButtonTappedCallback = { [weak self] in
                            self?.presentMonthVC()
                        }
                    }
                }
